...
#' @param theme Theme name or o2f_theme object to apply to all tables.
#' @param parallel Logical. Use parallel processing if available.
#' @param verbose Logical. Print progress messages.
#' @param ... Additional arguments passed to all o2f() calls.
#'
...
o2f_batch <- function(data_list,
  ...
  is_df <- vapply(data_list, is.data.frame, logical(1))
  if (!all(is_df)) {
...
#'   defaults).
#'
#' @return A o2f_batch_spec object.
#'
#' @examples
#' \dontrun{
#' specs <- list(
...
#'
#' @export
o2f_batch_spec <- function(df, filename, ...) {
  if (!is.data.frame(df)) {
...
#'   settings.
#'
#' @param specs A list of o2f_batch_spec objects.
#' @param sub_dir Default output directory.
#' @param theme Default theme (can be overridden per table).
#' @param parallel Use parallel processing.
#' @param verbose Print progress messages.
#' @param ... Default arguments passed to all o2f() calls.
#'
#' @return A named list of output file paths.
#'
#' @export
o2f_batch_advanced <- function(specs,
                               sub_dir = get_default_figures_dir(),
                               theme = NULL,
                               parallel = FALSE,
                               verbose = FALSE,
  ...
  # Validate all specs
  is_spec <- sapply(specs, inherits, "o2f_batch_spec")
  if (!all(is_spec)) {
    stop("All elements must be o2f_batch_spec objects.", call. = FALSE)
  }
