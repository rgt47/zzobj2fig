...
# o2f_siunitx creates valid column specification
spec <- o2f_siunitx(table_format = "3.2")
expect_inherits(spec, "o2f_siunitx")
expect_true(grepl("^S\\[", as.character(spec)))
expect_true(grepl("table-format=3.2", as.character(spec)))

# o2f_siunitx includes required packages
packages <- attr(spec, "packages")
expect_true(any(grepl("siunitx", packages)))
expect_true(any(grepl("detect-all", packages)))

# o2f_decimal is convenience wrapper
spec <- o2f_decimal(2, 3)
expect_inherits(spec, "o2f_siunitx")
...
align <- list(
  "l",
  o2f_siunitx(table_format = "3.2"),
  "r",
  o2f_siunitx(table_format = "2.3")
)
result <- zzobj2fig:::detect_siunitx_columns(align)
...
result <- zzobj2fig:::protect_siunitx_headers(latex_table, c(2, 3))
...
result <- zzobj2fig:::protect_siunitx_headers(latex_table, c(2))
...
# o2f_footnote creates valid object
fn <- o2f_footnote(general = "Note text")
expect_inherits(fn, "o2f_footnote")
expect_equal(fn$general, "Note text")
expect_true(fn$threeparttable)

# o2f_footnote accepts multiple notation types
fn <- o2f_footnote(
...
# o2f_mark creates footnote marker
result <- o2f_mark("23.5", 1, "symbol")
...
# o2f_header_above creates valid header object
hdr <- o2f_header_above(" " = 1, "Group A" = 2, "Group B" = 2)
expect_inherits(hdr, "o2f_header")
expect_equal(sum(hdr$header), 5)
expect_true(hdr$bold)

# o2f_header_above rejects invalid input
expect_error(
  o2f_header_above("Group A", "Group B"),
...
# o2f_collapse_rows creates valid object
collapse <- o2f_collapse_rows(1, valign = "top")
expect_inherits(collapse, "o2f_collapse")
expect_equal(collapse$columns, 1)
expect_equal(collapse$valign, "top")


# --- Integration: o2f with siunitx (requires LaTeX) ---
