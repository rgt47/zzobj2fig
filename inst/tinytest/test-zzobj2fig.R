...
# o2f rejects invalid input type
expect_error(o2f("not a dataframe"), "No o2f method for class")

# o2f rejects empty dataframe
expect_error(o2f(data.frame()), "`df` must not be empty")

# o2f rejects invalid scolor
test_df <- data.frame(a = 1:3, b = letters[1:3])
expect_error(
  o2f(test_df, scolor = c("red", "blue")),
  "`scolor` must be a single character value"
)

# o2f rejects invalid verbose
expect_error(
  o2f(test_df, verbose = "true"),
  "`verbose` must be a single logical value"
)

# o2f rejects invalid directory
expect_error(o2f(test_df, sub_dir = NULL), "Directory name cannot be NULL")
expect_error(o2f(test_df, sub_dir = ""), "Directory name cannot be empty")
...
# o2f handles basic dataframe conversion
...
# o2f handles special characters in column names
test_df2 <- data.frame(
  `col #1` = 1:3,
  `col %2` = letters[1:3],
  check.names = FALSE,
