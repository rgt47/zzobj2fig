...
o2f_inline <- function(x,
  ...
  if (is.null(filename)) {
    filename <- paste0("o2f_inline_", format(Sys.time(), "%Y%m%d%H%M%S"),
  ...
  pdf_path <- o2f(x, filename = filename, sub_dir = sub_dir, ...)
...
build_inline_latex <- function(path, width, height, align,
  ...
  if (has_frame || has_background) {
    ...
    if (has_frame) {
      ...
      bg_color <- if (has_background) background else "white"
      include_cmd <- paste0(
        "{", box_setup,
        "\\fcolorbox{", frame_color, "}{", bg_color, "}{",
        include_cmd, "}}"
      )
    } else {
      include_cmd <- paste0(
...
#' @param inner_sep Padding between content and frame. Default "2pt".
#' @param ... Additional arguments passed to o2f_inline().
#'
#' @return Same as o2f_inline().
#'
#' @examples
#' \dontrun{
