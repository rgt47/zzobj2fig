...
#'   conjunction with \code{zzo2f(..., compile = FALSE)}.
#'
#' @param source A \code{zzo2f_source} object (returned by
#'   \code{zzo2f(..., compile = FALSE)}) or a character vector of
#'   Typst source lines.
...
zzo2f_compile <- function(source,
  ...
  if (inherits(source, "zzo2f_source")) {
    ...
    stop(
      "`source` must be a zzo2f_source object or character vector.",
      call. = FALSE
    )
  }
