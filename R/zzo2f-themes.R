...
#'
#' @description Translates o2f theme objects and LaTeX styling parameters
...
#'
#' @description Takes a theme argument (NULL, character, or o2f_theme object),
#'   resolves it via the existing theme system, then translates LaTeX-specific
#'   fields to Typst-compatible values.
#'
#' @param theme Theme argument from zzo2f() call.
...
#' Translate o2f_footnote to tinytable notes
#'
#' @description Converts a o2f_footnote object into a character vector
#'   suitable for `tinytable::tt(notes=)`.
#'
#' @param footnote A o2f_footnote object or NULL.
#' @return Character vector of footnote strings, or NULL.
#' @keywords internal
translate_footnote <- function(footnote) {
  if (is.null(footnote)) return(NULL)
  if (!inherits(footnote, "o2f_footnote")) {
...
#'
#' @description Converts a o2f_header object (or list of them) into a named
...
translate_header_above <- function(header_above) {
  ...
  if (inherits(header_above, "o2f_header")) {
