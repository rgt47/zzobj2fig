#' Structural Specifications for zzo2f Tables
#'
#' @description Spec constructors for spanning headers and collapsed
#'   (merged) rows. Consumed by \code{zzo2f()} via the
#'   \code{header_above} and \code{collapse_rows} parameters.
#'
#' @name zzo2f-structure
NULL

#' Spanning column headers (Typst backend)
#'
#' @description Thin constructor that delegates to
#'   \code{o2f_header_above()}. The underlying \code{o2f_header} spec
#'   is backend-agnostic; \code{zzo2f()} already consumes it via
...
#' }
#' @export
zzo2f_header_above <- function(..., bold = TRUE, italic = FALSE,
                               align = "c", line = TRUE,
                               line_sep = 3) {
  o2f_header_above(
...
#'   columns, matching \code{o2f_collapse_rows()} behavior.
...
#'   or \code{"none"}.
#' @return A \code{zzo2f_collapse} object.
#' @examples
#' \dontrun{
#' df <- data.frame(
...
#' }
#' @export
zzo2f_collapse_rows <- function(columns = NULL,
  ...
  structure(
    ...
    class = "zzo2f_collapse"
  )
}

#' Print method for zzo2f_collapse
#' @param x A \code{zzo2f_collapse} object.
#' @param ... Ignored.
#' @return Invisibly \code{x}.
#' @export
print.zzo2f_collapse <- function(x, ...) {
  cat("zzo2f collapse specification:\n")
  cat("  columns: ",
...
#' @param spec A \code{zzo2f_collapse} object or NULL.
#' @return A list with elements \code{data} and \code{boundaries}.
#' @keywords internal
apply_zzo2f_collapse <- function(data, spec) {
  if (is.null(spec)) return(list(data = data, boundaries = integer(0)))
  if (!inherits(spec, "zzo2f_collapse")) {
    stop(
      "`collapse_rows` must be a zzo2f_collapse spec or NULL.",
      call. = FALSE
    )
  }
