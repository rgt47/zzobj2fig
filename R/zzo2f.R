...
#'   the LaTeX-based \code{\link{o2f}} pipeline, requiring only Typst (~40 MB
...
#'   \item Resolve theme via existing o2f theme system, translate to Typst
...
#' }
#'
#' Parameters from \code{o2f()} that are dropped (LaTeX-specific):
...
#' @param footnote A o2f_footnote object.
...
zzo2f.default <- function(x,
  ...
  if (!is.data.frame(x) && !is.matrix(x) && !inherits(x, "table")) {
    stop("No zzo2f method for class '", class(x)[1],
...
#' @rdname zzo2f.default
#' @export
zzo2f.data.frame <- function(x, ...) {
  zzo2f.default(x, ...)
}

#' @rdname zzo2f.default
#' @export
zzo2f.matrix <- function(x, ...) {
  zzo2f.default(x, ...)
}

#' @rdname zzo2f.default
#' @export
zzo2f.table <- function(x, ...) {
  zzo2f.default(x, ...)
}
...
#' @param ... Additional arguments passed to zzo2f.default.
#'
#' @return Invisibly returns the path to the output file.
#' @export
zzo2f.lm <- function(x,
...
#' zzo2f method for ANOVA objects
#'
...
#' @return Invisibly returns the path to the output file.
#' @export
zzo2f.anova <- function(x, digits = 3, ...) {
  df <- as.data.frame(x)
...
#' zzo2f method for aov objects
#' @rdname zzo2f.anova
#' @export
zzo2f.aov <- function(x, digits = 3, ...) {
  zzo2f.anova(summary(x)[[1]], digits = digits, ...)
}

#' zzo2f method for hypothesis tests
#'
...
#' @return Invisibly returns the path to the output file.
#' @export
zzo2f.htest <- function(x, digits = 3, ...) {
  result <- data.frame(
...
#' @param footnote o2f_footnote or NULL.
#' @param header_above o2f_header or NULL.
#' @param format Output format.
#' @param dpi PNG resolution.
#' @param ... Passed to tt().
#' @return Output file path (invisible).
#' @keywords internal
zzo2f_internal <- function(x,
                           filename,
                           sub_dir,
                           verbose,
                           caption,
  ...
  if (!is.null(header_above) &&
    ...
    stop(
      "`header_above` must be a o2f_header object, list, or NULL.",
      call. = FALSE
    )
  }
  ...
  if (has_rownames) {
    ...
    if (!is.null(header_above) && inherits(header_above, "o2f_header")) {
  ...
  if (!is.null(collapse_rows)) {
    cr <- apply_zzo2f_collapse(x, collapse_rows)
    x <- cr$data
  ...
  tbl <- apply_zzo2f_formats(tbl, formats, x)
  ...
  if (isFALSE(compile)) {
    ...
    return(structure(
      typ_content,
      class = c("zzo2f_source", "character"),
      typ_file = typ_file,
      filename = filename,
      sub_dir = sub_dir,
      format = format,
...
#' Check Typst dependencies for zzo2f
#'
...
check_typst_deps <- function() {
  message("Checking Typst dependencies for zzo2f()...\n")
  ...
  if (ready) {
    message("\nAll dependencies available. zzo2f() is ready to use.")
  } else {
