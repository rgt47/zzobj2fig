...
#'
#' @description Shared utility functions used across the zzobj2fig package.
#'
#' @name utils
#' @keywords internal
NULL
...
#' Assert parameter meets type requirements
#' @param x Value to check.
#' @param name Parameter name for error message.
#' @param type Expected type ("logical", "character").
#' @param allow_null Logical. Allow NULL values.
#' @param single Logical. Require single value.
#' @keywords internal
assert_type <- function(x, name, type, allow_null = FALSE, single = TRUE) {
  if (is.null(x)) {
    if (!allow_null) {
  ...
  if (!type_check || (single && length(x) != 1)) {
    null_msg <- if (allow_null) " or NULL" else ""
    len_msg <- if (single) "a single " else ""
    stop("`", name, "` must be ", len_msg, type, " value", null_msg, ".",
         call. = FALSE)
  }
  invisible(x)
}
...
assert_single_logical <- function(x, name) {
  assert_type(x, name, "logical", allow_null = FALSE, single = TRUE)
}
...
assert_string_or_null <- function(x, name) {
  assert_type(x, name, "character", allow_null = TRUE, single = TRUE)
}
...
assert_single_string <- function(x, name) {
  assert_type(x, name, "character", allow_null = FALSE, single = TRUE)
}
