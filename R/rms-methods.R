...
#'
#' @export
o2f.ols <- function(x, digits = 3, conf.int = TRUE, conf.level = 0.95,
                    output = c("coef", "anova"), ...) {
...
#'
#' @export
o2f.lrm <- function(x, digits = 3, exponentiate = TRUE,
...
#'
#' @export
o2f.cph <- function(x, digits = 3, exponentiate = TRUE,
                    conf.int = TRUE, conf.level = 0.95,
                    output = c("coef", "anova"), ...) {
  output <- match.arg(output)
...
#'
#' @export
o2f.orm <- function(x, digits = 3, exponentiate = TRUE,
                    conf.int = TRUE, conf.level = 0.95,
                    output = c("coef", "anova"),
                    intercepts = FALSE, ...) {
  output <- match.arg(output)
...
#' o2f method for rms generalized linear models
#'
...
#'
#' @export
o2f.Glm <- function(x, digits = 3, exponentiate = FALSE,
                    conf.int = TRUE, conf.level = 0.95,
                    output = c("coef", "anova"), ...) {
  output <- match.arg(output)
...
#'
#' @export
o2f.psm <- function(x, digits = 3, conf.int = TRUE, conf.level = 0.95,
                    output = c("coef", "anova"), ...) {
  output <- match.arg(output)
...
#'   regression models side-by-side, similar to o2f_regression() but
...
o2f_rms_compare <- function(...,
  ...
  do.call(o2f.default, c(list(x = result, filename = filename,
                              sub_dir = sub_dir), o2f_args))
}
