...
#'   extend zzobj2fig's support to survival models, time series, clustering,
...
#' @param ... Additional arguments passed to o2f().
#'
...
#'
#' @export
o2f_tidy <- function(x, tidy_args = list(), glance = FALSE, digits = 3, ...) {
  check_broom()
...
#'
#' @export
o2f.coxph <- function(x, digits = 3, exponentiate = TRUE,
...
#'
#' @export
o2f.survreg <- function(x, digits = 3, conf.int = TRUE,
                        conf.level = 0.95, ...) {
  check_broom()
...
#' o2f method for survfit (survival curves)
#'
...
#'
#' @export
o2f.survfit <- function(x, digits = 3, times = NULL, ...) {
  check_broom()
...
#' o2f method for survdiff (survival difference tests)
#'
...
#'
#' @export
o2f.survdiff <- function(x, digits = 3, ...) {
  check_broom()
...
#'
#' @export
o2f.Arima <- function(x, digits = 3, conf.int = TRUE,
                      conf.level = 0.95, ...) {
  check_broom()
...
#' o2f method for nonlinear least squares models
#'
...
#'
#' @export
o2f.nls <- function(x, digits = 3, conf.int = TRUE,
                    conf.level = 0.95, ...) {
  check_broom()
...
#'
#' @export
o2f.polr <- function(x, digits = 3, exponentiate = FALSE,
                     conf.int = TRUE, conf.level = 0.95, ...) {
  check_broom()
...
#'
#' @export
o2f.multinom <- function(x, digits = 3, exponentiate = FALSE,
                         conf.int = TRUE, conf.level = 0.95, ...) {
  check_broom()
...
#'
#' @export
o2f.prcomp <- function(x, matrix = "rotation", digits = 3, ...) {
  if (matrix == "summary") {
    s <- summary(x)
...
#'
#' @export
o2f.kmeans <- function(x, matrix = "centers", digits = 3, ...) {
  if (matrix == "centers") {
...
#'
#' @export
o2f.lmerMod <- function(x, effects = "fixed", digits = 3,
                        conf.int = TRUE, conf.level = 0.95, ...) {
...
#'
#' @inheritParams o2f.lmerMod
#' @param exponentiate Logical. Exponentiate coefficients.
#'
#' @return Invisibly returns the path to the generated PDF.
#'
#' @export
o2f.glmerMod <- function(x, effects = "fixed", digits = 3,
...
#'
#' @export
o2f.lme <- function(x, effects = "fixed", digits = 3,
                    conf.int = TRUE, conf.level = 0.95, ...) {
