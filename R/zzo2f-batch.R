...
#'
#' @description Parallel to \code{o2f_batch()} / \code{o2f_batch_advanced()}
#'   / \code{o2f_batch_spec()}: render many tables in one call,
#'   routing through \code{zzo2f()} rather than \code{o2f()}.
#'
#' @name zzo2f-batch
NULL
...
#' }
#' @export
zzo2f_batch <- function(data_list,
...
#' @param ... Per-table overrides passed to \code{zzo2f()}.
#' @return A \code{zzo2f_batch_spec} object.
#' @examples
#' \dontrun{
#' specs <- list(
...
#' }
#' @export
zzo2f_batch_spec <- function(df, filename, ...) {
  if (!is.data.frame(df)) {
  ...
  structure(
    ...
    class = "zzo2f_batch_spec"
  )
}

#' Advanced batch rendering with per-table overrides
#'
#' @param specs A list of \code{zzo2f_batch_spec} objects.
#' @param sub_dir Default output directory.
#' @param theme Default theme.
#' @param parallel Logical.
#' @param verbose Logical.
#' @param ... Default arguments passed to each \code{zzo2f()} call.
#' @return Named list of output file paths.
#' @export
zzo2f_batch_advanced <- function(specs,
                                 sub_dir = get_default_figures_dir(),
                                 theme = NULL,
                                 parallel = FALSE,
                                 verbose = FALSE,
  ...
  is_spec <- vapply(specs, inherits, logical(1L), "zzo2f_batch_spec")
  if (!all(is_spec)) {
    stop(
      "All elements must be zzo2f_batch_spec objects.",
      call. = FALSE
    )
  }
...
#' Print method for zzo2f_batch_spec
#' @param x A \code{zzo2f_batch_spec} object.
#' @param ... Ignored.
#' @return Invisibly \code{x}.
#' @export
print.zzo2f_batch_spec <- function(x, ...) {
  cat("zzo2f batch spec:\n")
