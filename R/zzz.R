...
.onLoad <- function(libname, pkgname) {
  # Register the o2f and zzo2f knitr engines if knitr is available
...
.onAttach <- function(libname, pkgname) {
  ...
  packageStartupMessage(
    "zzobj2fig ", pkg_version, " - LaTeX table generation for R"
  )

  # Check for required system dependencies
  has_pdflatex <- command_exists("pdflatex")
  has_pdfcrop <- command_exists("pdfcrop")

  if (!has_pdflatex) {
    packageStartupMessage(
      "Note: pdflatex not found. Install a LaTeX distribution to use o2f()."
    )
  }
...
ensure_pdfcrop <- function(auto_install = TRUE, verbose = TRUE) {
  ...
  if (!has_tinytex_pkg) {
    warning(
      ...
      "Option 3 - Use o2f() without cropping:\n",
      "  o2f(data, crop = FALSE)",
      call. = FALSE
    )
  ...
  if (!has_tinytex_dist) {
    ...
    warning(
      ...
      "  tinytex::install_tinytex()\n",
      "  tinytex::tlmgr_install('pdfcrop')\n\n",
      "Or use o2f() without cropping: o2f(data, crop = FALSE)",
      call. = FALSE
    )
    return(invisible(FALSE))
  }
...
check_latex_deps <- function(auto_install = TRUE) {
  message("Checking LaTeX dependencies for zzobj2fig...\n")
