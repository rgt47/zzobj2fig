# Tests for zzo2f Batch D figure-inclusion helpers.

# --- Path resolution ---

expect_equal(
  zzobj2fig:::resolve_typst_path("figures/demo"),
  "figures/demo.pdf"
)
expect_equal(
  zzobj2fig:::resolve_typst_path("figures/demo.pdf"),
  "figures/demo.pdf"
)
expect_equal(
  zzobj2fig:::resolve_typst_path("out/x.png"),
  "out/x.pdf"
)

# --- Width translation ---

expect_equal(zzobj2fig:::translate_width("\\textwidth"), "100%")
expect_equal(zzobj2fig:::translate_width("0.8\\textwidth"), "80%")
expect_equal(zzobj2fig:::translate_width("0.48\\textwidth"), "48%")
expect_equal(zzobj2fig:::translate_width("5cm"), "5cm")
expect_equal(zzobj2fig:::translate_width("80%"), "80%")
expect_equal(zzobj2fig:::translate_width(NULL), "100%")
...
# --- zzo2f_margin_packages returns NULL with a message ---

msg <- capture.output(
  res <- zzo2f_margin_packages("sidenotes"),
  type = "message"
)
expect_null(res)
expect_true(any(grepl("Typst requires no margin packages", msg)))

# --- End-to-end: embed a zzo2f-rendered table in a host Typst doc ---

has_typst <- nzchar(Sys.which("typst"))
has_tt <- requireNamespace("tinytable", quietly = TRUE)

if (has_typst && has_tt) {
  tmp <- file.path(tempdir(), "zzo2f_include_test")
  dir.create(tmp, showWarnings = FALSE, recursive = TRUE)

  # First, render a table with zzo2f
  tab <- zzo2f(
    mtcars[1:3, 1:3],
    filename = "inner",
    sub_dir = tmp,
