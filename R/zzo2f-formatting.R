...
#'
#' @return An S3 object of class \code{"zzo2f_format"}.
#' @examples
#' \dontrun{
...
#' }
#' @export
zzo2f_format <- function(rows = NULL,
                         cols = NULL,
                         bold = FALSE,
                         italic = FALSE,
                         color = NULL,
...
#' @return A \code{zzo2f_format} spec.
#' @examples
#' \dontrun{
#' zzo2f_bold_col(c("estimate", "p_value"))
#' }
#' @export
zzo2f_bold_col <- function(cols) {
  if (is.null(cols) || length(cols) == 0L) {
    stop("`cols` must be a non-empty vector.", call. = FALSE)
  }
  zzo2f_format(cols = cols, bold = TRUE)
}
...
#' @examples
#' \dontrun{
#' zzo2f_italic_col("variable")
#' }
#' @export
zzo2f_italic_col <- function(cols) {
  if (is.null(cols) || length(cols) == 0L) {
    stop("`cols` must be a non-empty vector.", call. = FALSE)
  }
  zzo2f_format(cols = cols, italic = TRUE)
}
...
#' @examples
#' \dontrun{
#' zzo2f_color_row(c(1, 3, 5), "blue!10")
#' }
#' @export
zzo2f_color_row <- function(rows, background) {
  ...
  zzo2f_format(rows = rows, background = background)
}
...
#' }
#' @export
zzo2f_highlight <- function(condition,
...
#'
#' @description Produces a \code{zzo2f_format} spec that rounds and
...
#' @return A \code{zzo2f_format} spec with \code{digits} slot attached.
#' @examples
#' \dontrun{
#' zzo2f_decimal(c("estimate", "std.error"), digits = 2)
#' }
#' @export
zzo2f_decimal <- function(cols, digits = 3L) {
  if (is.null(cols) || length(cols) == 0L) {
    stop("`cols` must be a non-empty vector.", call. = FALSE)
  }
  ...
  spec <- zzo2f_format(cols = cols)
  spec$digits <- as.integer(digits)
  class(spec) <- c("zzo2f_decimal", "zzo2f_format")
  spec
}

#' Print method for zzo2f_format
#' @param x A \code{zzo2f_format} object.
#' @param ... Ignored.
#' @return Invisibly \code{x}.
#' @export
print.zzo2f_format <- function(x, ...) {
  cat("zzo2f format specification:\n")
  if (!is.null(x$rows)) {
  ...
  if (inherits(x, "zzo2f_decimal")) cat("  digits:", x$digits, "\n")
  invisible(x)
}
...
#' Apply a single zzo2f_format spec to a tinytable object
#'
#' @param tbl A \code{tinytable} object.
#' @param spec A \code{zzo2f_format} spec.
#' @param data The data frame backing \code{tbl}.
#' @return Modified \code{tinytable} object.
#' @keywords internal
apply_zzo2f_format <- function(tbl, spec, data) {
  j_idx <- resolve_cols(spec$cols, data)
  i_idx <- spec$rows

  if (inherits(spec, "zzo2f_decimal")) {
    if (!is.null(j_idx)) {
...
#' Apply a list of zzo2f_format specs to a tinytable object
#'
#' @param tbl A \code{tinytable} object.
#' @param formats A single \code{zzo2f_format}, a list of them, or
#'   NULL.
#' @param data The data frame backing \code{tbl}.
#' @return Modified \code{tinytable} object.
#' @keywords internal
apply_zzo2f_formats <- function(tbl, formats, data) {
  if (is.null(formats)) return(tbl)
  if (inherits(formats, "zzo2f_format")) formats <- list(formats)
  if (!is.list(formats)) {
    stop(
      "`formats` must be a zzo2f_format spec, a list of them, ",
      "or NULL.",
      call. = FALSE
    )
  }
  for (spec in formats) {
    if (!inherits(spec, "zzo2f_format")) {
      stop(
        "Every element of `formats` must be a zzo2f_format spec.",
        call. = FALSE
      )
    }
    tbl <- apply_zzo2f_format(tbl, spec, data)
  }
  tbl
}
