...
#'
#' @description Generates a PDF or PNG via \code{zzo2f()} and, when
...
#'   helpers; \code{zzo2f()} itself does not consume it.
...
#' @keywords internal
zzo2f_engine <- function(options) {
  if (!requireNamespace("knitr", quietly = TRUE)) {
    stop("knitr is required for the zzo2f engine.", call. = FALSE)
  }
  ...
  zzt_opts <- list()
  if (!is.null(options$zzo2f.caption)) {
    zzt_opts$caption <- options$zzo2f.caption
  }
  if (!is.null(options$zzo2f.theme)) {
    zzt_opts$theme <- options$zzo2f.theme
  }
  if (!is.null(options$zzo2f.align)) {
    zzt_opts$align <- options$zzo2f.align
  }
  if (!is.null(options$zzo2f.scolor)) {
    zzt_opts$scolor <- options$zzo2f.scolor
  }
  output_format <- options$zzo2f.output_format %||% "pdf"

  filename <- options$label %||%
    paste0("zzo2f_", format(Sys.time(), "%H%M%S"))
  ...
  out <- sprintf(
    "![%s](%s)", options$zzo2f.caption %||% "", output_path
  )
...
#' @export
register_zzo2f_engine <- function() {
  if (requireNamespace("knitr", quietly = TRUE)) {
    knitr::knit_engines$set(zzo2f = zzo2f_engine)
  }
  invisible(NULL)
}
