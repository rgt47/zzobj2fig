...
#'   or passed to \code{zzo2f(..., preamble = ...)} when that
...
#' exist so that code migrating from \code{o2f_*} has
...
#' @export
zzo2f_textlang <- function(language) {
...
#' @export
zzo2f_font <- function(main_font = NULL,
                       mono_font = NULL,
                       sans_font = NULL) {
  out <- character(0)
  ...
  if (length(out) == 0L) {
    message(
      "zzo2f_font() called with no font specified; returning ",
      "empty vector."
    )
  }
  out
...
#' @export
zzo2f_page <- function(margin = NULL, paper = NULL,
                       landscape = FALSE,
