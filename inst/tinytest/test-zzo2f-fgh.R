# Tests for zzo2f Batches F (stat), G (preamble), H (siunitx).
...
if (has_broom && has_typst && has_tt) {
  tmp <- file.path(tempdir(), "zzo2f_stat_test")
...
# zzo2f_rms_compare requires rms or, lacking it, errors cleanly.
if (requireNamespace("rms", quietly = TRUE)) {
  expect_error(zzo2f_rms_compare(), "At least one model")
} else {
  expect_error(zzo2f_rms_compare(), "Package 'rms' is required")
}
...
expect_equal(zzo2f_textlang("de"), '#set text(lang: "de")')
expect_error(zzo2f_textlang(""), "non-empty")
expect_error(zzo2f_textlang(c("a", "b")), "single non-empty")

# zzo2f_font
out <- zzo2f_font(main_font = "New Computer Modern")
expect_equal(out, '#set text(font: "New Computer Modern")')

out2 <- zzo2f_font(main_font = "Source Serif",
                    mono_font = "Fira Code")
...
msg <- capture.output(empty <- zzo2f_font(), type = "message")
expect_equal(length(empty), 0L)

# zzo2f_page
expect_equal(zzo2f_page(), "#set page()")
expect_equal(
  zzo2f_page(paper = "a4"),
  '#set page(paper: "a4")'
)
expect_equal(
  zzo2f_page(paper = "us-letter", landscape = TRUE),
  '#set page(paper: "us-letter", flipped: true)'
)
expect_equal(
  zzo2f_page(margin = "2cm"),
  "#set page(margin: 2cm)"
)
expect_equal(
  zzo2f_page(margin = list(x = "2cm", y = "1.5cm")),
  "#set page(margin: (x: 2cm, y: 1.5cm))"
)
expect_equal(
  zzo2f_page(width = "auto", height = "auto"),
  "#set page(width: auto, height: auto)"
)
expect_error(
  zzo2f_page(margin = list("2cm", "1cm")),
  "fully named"
)
...
if (has_typst) {
  tmp <- file.path(tempdir(), "zzo2f_preamble_test")
  dir.create(tmp, showWarnings = FALSE, recursive = TRUE)
  src <- c(
    zzo2f_page(paper = "a4",
...
spec <- zzo2f_siunitx(table_format = "3.2")
expect_inherits(spec, "zzo2f_siunitx")
...
# Input validation
expect_error(
  zzo2f_siunitx(table_format = "3"),
  "<int>\\.<dec>"
)
expect_error(
  zzo2f_siunitx(round_mode = "figures"),
  "requires `round_precision`"
)

# Print method runs without error
out <- capture.output(print(spec2))
expect_true(any(grepl("zzo2f_siunitx", out)))
