...
#'
#' @description Returns the path to the o2f cache directory. Creates it if
#'   it doesn't exist.
#'
...
#'
#' @export
o2f_cache_dir <- function(create = TRUE) {
  cache_dir <- getOption(
    "zzobj2fig.cache_dir",
    file.path(tempdir(), "zzobj2fig_cache")
  )
...
#'
#' @export
o2f_cache_set_dir <- function(path) {

  old <- getOption("zzobj2fig.cache_dir")

  if (is.null(path)) {
    options(zzobj2fig.cache_dir = NULL)
  } else {
    if (!is.character(path) || length(path) != 1) {
      stop("`path` must be a single character string or NULL.", call. = FALSE)
    }
    options(zzobj2fig.cache_dir = path)
  }
...
#'
#' @description Compute a hash of the inputs to o2f for use as a cache key.
#'
...
compute_cache_hash <- function(df, scolor = NULL, extra_packages = NULL,
  ...
  theme_name <- if (inherits(theme, "o2f_theme")) theme$name else theme
...
check_cache <- function(hash, output_format = "pdf") {
  cache_dir <- o2f_cache_dir(create = FALSE)
...
store_in_cache <- function(source_path, hash) {
  ...
  cache_dir <- o2f_cache_dir(create = TRUE)
...
retrieve_from_cache <- function(cache_path, target_path) {
  ...
  isTRUE(file.copy(cache_path, target_path, overwrite = TRUE))
}
...
#'
#' @export
o2f_cache_clear <- function(older_than = NULL) {
  cache_dir <- o2f_cache_dir(create = FALSE)
...
#'
#' @description Report on the current state of the o2f cache.
#'
