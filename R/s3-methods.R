...
#'
#' @export
o2f.default <- function(x, filename = NULL,
                        ...
                        ...) {
  if (!is.data.frame(x)) {
    stop("No o2f method for class '", class(x)[1],
...
#' @rdname o2f.default
#' @export
o2f.data.frame <- function(x, ...) {
  o2f.default(x, ...)
}

#' o2f method for matrix
#'
#' @param x A matrix to convert to a LaTeX table.
#' @param rownames Logical. Include row names as first column.
#' @param ... Additional arguments passed to o2f.default.
#'
#' @return Invisibly returns the path to the generated PDF.
#'
#' @export
o2f.matrix <- function(x, rownames = TRUE, ...) {
  df <- as.data.frame(x)
...
#' o2f method for aov objects
#' @rdname o2f.anova
#' @export
o2f.aov <- function(x, digits = 3, ...) {
  o2f.anova(summary(x)[[1]], digits = digits, ...)
}
...
#' @param o2f_args List of additional arguments passed to o2f().
#'
#' @return Invisibly returns the path to the generated PDF.
#'
#' @examples
...
o2f_regression <- function(...,
  ...
  # Call o2f with the comparison table
  do.call(o2f.default, c(list(x = result, filename = filename, sub_dir = sub_dir),
                         o2f_args))
}
