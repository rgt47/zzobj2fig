#' Internal implementation for o2f
#'
...
#'   be stored. Defaults to "analysis/figures" in zzcollab projects (detected
#'   by .zzcollab/ or zzcollab.yaml), "figures" otherwise.
...
#'   o2f_siunitx objects for decimal alignment.
...
#' @param theme A o2f_theme object or character string naming a built-in theme
#'   ("minimal", "apa", "nature", "nejm"). Theme settings are used as defaults
#'   but can be overridden by explicit arguments.
#' @param footnote A o2f_footnote object created by o2f_footnote(). Specifies
#'   table footnotes with various notation styles.
#' @param header_above A o2f_header object or list of them, created by
#'   o2f_header_above(). Specifies spanning column headers.
#' @param collapse_rows A o2f_collapse object created by o2f_collapse_rows().
...
#'
#' @description Returns "analysis/figures" for zzcollab projects,
#'   "figures" otherwise. Detects zzcollab by presence of .zzcollab/
#'   directory or zzcollab.yaml file.
#' @return Character string with default figures directory path.
#' @keywords internal
get_default_figures_dir <- function() {
  if (dir.exists(".zzcollab") || file.exists("zzcollab.yaml")) {
    "analysis/figures"
  } else {
    "figures"
  }
...
#' @keywords internal
o2f_internal <- function(df, filename = NULL,
  ...
  if (!is.null(footnote) && !inherits(footnote, "o2f_footnote")) {
    stop("`footnote` must be a o2f_footnote object or NULL.", call. = FALSE)
  }
  if (!is.null(header_above) &&
      !inherits(header_above, "o2f_header") &&
      !is.list(header_above)) {
    stop("`header_above` must be a o2f_header object, list, or NULL.",
      call. = FALSE)
  }
  if (!is.null(collapse_rows) && !inherits(collapse_rows, "o2f_collapse")) {
    stop("`collapse_rows` must be a o2f_collapse object or NULL.", call. = FALSE)
  }
...
sanitize_filename <- function(filename) {
  result <- gsub("[^a-zA-Z0-9_]", "_", filename)
...
#' @param footnote A o2f_footnote object for table footnotes.
#' @param header_above A o2f_header object or list for spanning headers.
#' @param collapse_rows A o2f_collapse object for multi-row cells.
#' @keywords internal
create_latex_table <- function(df, tex_file, scolor, extra_packages = NULL,
  ...
  use_threeparttable <- !is.null(footnote) &&
    inherits(footnote, "o2f_footnote") &&
