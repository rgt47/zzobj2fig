# Advanced Features: zzobj2fig

zzobj2fig v0.2.0 introduced substantial enhancements for
publication-quality table formatting. This vignette demonstrates the
advanced capabilities that distinguish zzobj2fig from simpler table
packages:

- **Table footnotes** with multiple notation types (general, numbered,
  alphabetic, symbol)
- **Spanning headers** for grouped column layouts common in clinical
  tables
...
These features address requirements frequently encountered in medical,
scientific, and social science publications.
...
Professional tables often require footnotes to explain abbreviations,
provide context, or indicate significance levels. zzobj2fig provides
full footnote support via the
...
anova_results$F[1] <- o2f_mark("12.93", 1, "symbol")
anova_results$F[2] <- o2f_mark("6.66", 2, "symbol")
...
endpoints$Endpoint[1] <- o2f_mark(endpoints$Endpoint[1], 1, "number")
endpoints$Difference[4] <- o2f_mark(endpoints$Difference[4], 2, "number")

fn_numbered <- o2f_footnote(
  number = c(
...
Spanning headers group related columns under a common label, essential
for complex tables comparing multiple conditions or timepoints.
...
Proper decimal alignment makes numeric tables easier to read and
compare. zzobj2fig provides siunitx integration for precise alignment.
...
o2f(coefs,
    ...
    align = list(
      "l",
      o2f_siunitx(table_format = "1.3"),
      o2f_siunitx(table_format = "1.3"),
      o2f_siunitx(table_format = "2.2"),
      "r"
    ))
```
...
For documents with a List of Tables, short captions provide concise
entries while the full caption remains with the table.
...
Real-world tables often require multiple advanced features
simultaneously.
...
clinical$Diff <- sapply(seq_len(nrow(clinical)), function(i) {
  if (as.numeric(clinical$P[i]) < 0.05) {
    o2f_mark(as.character(clinical$Diff[i]), 1, "symbol")
  } else {
...
o2f(clinical,
    ...
    header_above = hdr,
    footnote = fn,
    collapse_rows = o2f_collapse_rows(columns = 1, valign = "top"),
    theme = "nejm")
```
...
function provides a streamlined workflow for including tables directly
in R Markdown documents without LaTeX float positioning.
...
The table appears exactly where the code chunk is placed, rather than
being positioned by LaTeX’s float algorithm.
...
- `caption`: Table caption using `\captionof{table}{...}` (no float
  required)
...
provides sensible defaults for coefficient tables:

``` r

model <- lm(mpg ~ cyl + hp + wt, data = mtcars)
o2f_coef(model, caption = "Model Coefficients", label = "tab:coef")
```

Defaults: `width = "3in"`, `align = "left"`,
`caption_position = "above"`
...
zzobj2fig requires LaTeX tools (`pdflatex`, `pdfcrop`) to generate PDFs.
Helper functions assist with checking and installing these dependencies.
...
This checks for the `tinytex` R package and TinyTeX distribution, then
runs `tinytex::tlmgr_install("pdfcrop")` if available.
...
After generating tables, use these helpers to include them in LaTeX
documents with precise placement control. These functions output raw
LaTeX code for use in `.Rnw` or `.tex` documents.

### Standard Figure Float

The
[`o2f_include()`](https://rgt47.github.io/zzobj2fig/reference/o2f_include.md)
function wraps the PDF in a figure environment:

``` r

o2f(mtcars[1:5, 1:4], filename = "demo_table", sub_dir = "tables")

o2f_include("tables/demo_table",
...
o2f_include_inline("tables/demo_table",
                   width = "0.8\\textwidth",
                   vspace = "1em")
```
...
o2f(mtcars[1:5, 1:3], filename = "table_a", sub_dir = "tables")
o2f(mtcars[1:5, 4:6], filename = "table_b", sub_dir = "tables")
...
o2f_include_margin("tables/demo_table",
                   caption = "Summary",
                   method = "sidenotes")
```
...
o2f_include_wrap("tables/demo_table",
                 placement = "r",
...
ref_text <- sprintf("Results are presented in %s.",
                    o2f_ref("tab:efficacy", cat = FALSE))
cat(ref_text)
```
...
zzobj2fig v0.2.0 provides comprehensive support for publication-quality
tables:

- **Inline tables**:
  [`o2f_inline()`](https://rgt47.github.io/zzobj2fig/reference/o2f_inline.md)
  for R Markdown without float positioning
- **Caption control**: `caption_position` (above/below) with
  `\captionof`
...
- **Decimal alignment**: siunitx integration for precise number
  formatting
- **Short captions**: For List of Tables entries
- **Figure placement**: Float, inline, margin, wrap, and side-by-side
  options
- **Dependency helpers**:
...
These features can be combined to create tables meeting the most
demanding journal requirements while maintaining a clean R-based
workflow.
