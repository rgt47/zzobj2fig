#' Advanced Table Features for zzobj2fig
#'
...
#'
#' @description Create footnote specifications for use with o2f(). Supports
...
#'
#' @return A o2f_footnote object (list with class "o2f_footnote").
#'
#' @details
#' Table footnotes in LaTeX require special handling. This function creates a
#' specification that o2f() uses to apply kableExtra's footnote() function.
#'
...
#'
#' @export
o2f_footnote <- function(general = NULL,
                         number = NULL,
                         alphabet = NULL,
                         symbol = NULL,
                         title_general = NULL,
...
#'
#' @export
o2f_mark <- function(text, mark, type = c("symbol", "number", "alphabet")) {
...
#' Print method for o2f_footnote objects
#'
#' @param x A o2f_footnote object.
#' @param ... Additional arguments (ignored).
#' @return Invisibly returns x.
#' @export
print.o2f_footnote <- function(x, ...) {
  cat("o2f footnote specification:\n")
...
#'
#' @return A o2f_header object (list with class "o2f_header").
#'
#' @details
...
#'
#' @export
o2f_header_above <- function(..., bold = TRUE, italic = FALSE,
...
#' Print method for o2f_header objects
#'
#' @param x A o2f_header object.
#' @param ... Additional arguments (ignored).
#' @return Invisibly returns x.
#' @export
print.o2f_header <- function(x, ...) {
  cat("o2f spanning header:\n")
...
#'
#' @return A o2f_collapse object (list with class "o2f_collapse").
#'
#' @examples
#' \dontrun{
...
#'
#' @export
o2f_collapse_rows <- function(columns = NULL,
...
#' Print method for o2f_collapse objects
#'
#' @param x A o2f_collapse object.
#' @param ... Additional arguments (ignored).
#' @return Invisibly returns x.
#' @export
print.o2f_collapse <- function(x, ...) {
  cat("o2f collapse rows specification:\n")
...
#'
#' @return A o2f_siunitx object containing the column specification and
...
#'
#' @export
o2f_siunitx <- function(table_format = "3.2",
...
#' Print method for o2f_siunitx objects
#'
#' @param x A o2f_siunitx object.
#' @param ... Additional arguments (ignored).
#' @return Invisibly returns x.
#' @export
print.o2f_siunitx <- function(x, ...) {
  cat("o2f siunitx column specification:\n")
  cat(" ", as.character(x), "\n")
...
#'
#' @export
o2f_decimal <- function(integers = 3, decimals = 2) {
  o2f_siunitx(table_format = paste0(integers, ".", decimals))
}
...
#' @param footnote_spec A o2f_footnote object.
#' @return Modified kable object with footnotes.
#' @keywords internal
apply_footnotes <- function(kable_obj, footnote_spec) {
  if (is.null(footnote_spec) || !inherits(footnote_spec, "o2f_footnote")) {
    return(kable_obj)
  }
...
#' @param kable_obj A kable object.
#' @param header_spec A o2f_header object or list of them.
...
apply_header_above <- function(kable_obj, header_spec) {
  ...
  if (inherits(header_spec, "o2f_header")) {
    header_spec <- list(header_spec)
  }

  for (hdr in header_spec) {
    if (!inherits(hdr, "o2f_header")) next
...
#' @param kable_obj A kable object.
#' @param collapse_spec A o2f_collapse object.
#' @return Modified kable object with collapsed rows.
#' @keywords internal
apply_collapse_rows <- function(kable_obj, collapse_spec) {
  if (is.null(collapse_spec) || !inherits(collapse_spec, "o2f_collapse")) {
    return(kable_obj)
  }
...
#'
#' @param align Alignment specification (may include o2f_siunitx objects).
...
process_alignment <- function(align) {
  ...
  for (i in seq_along(align)) {
    if (inherits(align[[i]], "o2f_siunitx")) {
