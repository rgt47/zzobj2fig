---
title: "Why zzobj2fig? A Practical Comparison with pander"
author: "Ronald G. Thomas"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Why zzobj2fig? A Practical Comparison with pander}
...
**zzobj2fig** through practical examples to help you choose the right tool.
...
**zzobj2fig** takes a different approach: it generates standalone PDF tables
...
- **Journal submissions and formal publications**: zzobj2fig provides the
...
```{r challenge1-zzobj2fig, eval=has_latex}
df <- mtcars[1:5, 1:4]
...
**Key difference**: zzobj2fig produces a PDF with professional typesetting
...
```{r challenge2-zzobj2fig, eval=has_latex}
df <- data.frame(
  Name = c("Alice", "Bob", "Charlie"),
  Score = c(95.5, 87.2, 91.8),
  Rank = c(1, 3, 2)
...
```{r challenge3-zzobj2fig, eval=has_latex}
df <- iris[1:5, ]
o2f(df,
...
**Key difference**: zzobj2fig adds LaTeX cross-reference labels for use with
...
```{r challenge4-zzobj2fig, eval=has_latex}
model <- lm(mpg ~ cyl + hp + wt, data = mtcars)
o2f(model,
    filename = "regression",
    sub_dir = output_dir,
...
**Key difference**: Both handle `lm` objects. zzobj2fig provides S3 methods
...
These challenges demonstrate capabilities unique to zzobj2fig. pander cannot
...
```{r challenge5-zzobj2fig, eval=has_latex}
df <- data.frame(
  Variable = c("BMI", "SBP", "DBP"),
  Mean = c(27.5, 142.3, 88.2),
  SD = c(4.2, 18.5, 11.3)
...
**Advantage**: zzobj2fig produces proper LaTeX footnotes using threeparttable,
...
```{r challenge6-zzobj2fig, eval=has_latex}
df <- data.frame(
  Outcome = c("Score A", "Score B"),
  T_Mean = c(45.2, 38.7),
  T_SD = c(8.3, 6.2),
...
```{r challenge7-zzobj2fig, eval=has_latex}
df <- data.frame(
  Category = c("Treatment", "Treatment", "Control", "Control"),
  Subgroup = c("Male", "Female", "Male", "Female"),
  N = c(45, 52, 48, 49),
...
**Advantage**: zzobj2fig automatically collapses repeated values using LaTeX
...
```{r challenge8-zzobj2fig, eval=has_latex}
df <- data.frame(
  Item = c("A", "B", "C"),
  Value1 = c(1.5, 123.45, 12.345),
  Value2 = c(0.001, 10.1, 1000.01)
...
```{r challenge9-zzobj2fig, eval=has_latex}
df <- data.frame(
  Characteristic = c("Age, years", "Male sex, n (%)", "BMI, kg/m2"),
  Treatment = c("65.2 (8.4)", "142 (58%)", "27.3 (4.1)"),
  Placebo = c("64.8 (8.1)", "138 (56%)", "27.1 (3.9)")
...
**Advantage**: zzobj2fig includes built-in themes for NEJM, APA, Nature, and
...
```{r challenge10-zzobj2fig, eval=has_latex}
m1 <- lm(mpg ~ cyl, data = mtcars)
m2 <- lm(mpg ~ cyl + hp, data = mtcars)
m3 <- lm(mpg ~ cyl + hp + wt, data = mtcars)
...
**Advantage**: `o2f_regression()` automatically aligns terms across models,
...
```{r challenge11-zzobj2fig, eval=has_latex}
df <- data.frame(
...
The `o2f_inline()` function is designed for exactly this use case:
...
This is equivalent to `o2f_inline()` with sensible defaults for coefficient
tables (width = "3in", align = "left").

**Note**: When using captions with `o2f_inline()`, add to your R Markdown YAML:
...
While pander supports 50+ object types natively, zzobj2fig provides `o2f_tidy()`
...
This means zzobj2fig can handle most statistical objects through broom, while
providing specialized formatting for the most common types (lm, glm, coxph,
lmer, rms objects, etc.).
...
- Need quick, inline display of many R object types (50+ supported natively)
...
This vignette demonstrates several features introduced in zzobj2fig v0.2.0:
- **o2f_inline()**: Inline tables for R Markdown without floats
- **o2f_coef()**: Quick coefficient tables with sensible defaults
- **caption_position**: Control caption placement (above/below)
- **S3 methods**: 15 native methods + extensible via broom and o2f_tidy()
- **Theme system**: Built-in NEJM theme (`theme = "nejm"`)
- **o2f_regression()**: Side-by-side model comparison
- **o2f_footnote()**: Structured footnotes with multiple notation types
- **o2f_header_above()**: Spanning column headers
- **o2f_collapse_rows()**: Multi-row cell merging
- **o2f_siunitx()**: Decimal alignment
- **o2f_mark()**: Footnote markers in cells
