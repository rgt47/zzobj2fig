...
The `zzobj2fig` package provides a streamlined workflow to transform R objects
into publication-ready LaTeX tables and generate cropped PDF outputs. With
built-in options for styling, directory management, and verbose logging, the
package is ideal for researchers, analysts, and data scientists who need
professional table outputs for reports, presentations, or publications.

## What's New in v0.2.0

Version 0.2.0 introduced substantial enhancements:

- **S3 method dispatch**: `o2f()` now works directly with 20+ object types:
  - Base: `lm`, `glm`, `anova`, `aov`, `htest`, `matrix`, `table`
  - Via broom: `coxph`, `survreg`, `survfit`, `nls`, `polr`, `prcomp`, `kmeans`,
    `lmerMod`, `glmerMod`, `lme`, `Arima`, and more
- **Theme system**: Built-in themes for NEJM, APA, Nature, and minimal styles
- **Inline workflow**: `o2f_inline()` for R Markdown without float positioning,
  with `\captionof` for proper table numbering
- **o2f_regression()**: Side-by-side regression model comparison tables
- **Advanced formatting**: Table footnotes, spanning headers, multi-row cells,
  and decimal alignment via siunitx
- **Caption control**: `caption_position` (above/below), short captions, and
  cross-reference labels
- **Dependency helpers**: `check_latex_deps()`, `ensure_pdfcrop()` for setup
...
- *Advanced Features* - Footnotes, headers, alignment, placement helpers, and
  inline workflow
- *Why zzobj2fig?* - Practical comparison with pander

This vignette covers the foundational workflow and customization options.
...
The primary function for converting R objects to LaTeX tables. In v0.2.0, `o2f()`
is an S3 generic with methods for data frames, matrices, tables, and statistical
objects.
...
- `scolor`: LaTeX color for row shading (e.g., `"blue!10"`). Default: `"blue!10"`.
...
- `header_above`: A `o2f_header_above` object for spanning column headers.
...
```{r include-basic-pdf, echo=FALSE, fig.cap="LaTeX Table Generated by zzobj2fig", out.width="100%"}
...
pdf_path <- "color_demo/blue_table_cropped.pdf"
if (file.exists(pdf_path)) {
  knitr::include_graphics(pdf_path)
} else {
  cat("PDF file not found:", pdf_path)
...
pdf_path <- "color_demo/green_table_cropped.pdf"
if (file.exists(pdf_path)) {
  knitr::include_graphics(pdf_path)
} else {
  cat("PDF file not found:", pdf_path)
...
For tables that should appear exactly where the code chunk is placed (without
LaTeX float positioning), use `o2f_inline()`:
...
And here's the professional LaTeX table generated by `o2f()`:

```{r include-demo-pdf, echo=FALSE, fig.cap="Professional LaTeX Table from o2f()", out.width="100%"}
pdf_path <- "tables/vignette_demo_cropped.pdf"
if (file.exists(pdf_path)) {
  knitr::include_graphics(pdf_path)
} else {
  cat("PDF file not found:", pdf_path)
