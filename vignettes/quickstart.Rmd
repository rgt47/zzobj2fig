---
title: "Quickstart Guide: zzobj2fig v0.2.1"
...
header-includes:
  ...
  - \usepackage{wrapfig}
vignette: >
  %\VignetteIndexEntry{Quickstart Guide: zzobj2fig v0.2.1}
...
include_table <- function(filename) {
  # Try new naming first (filename.pdf), fall back to old (_cropped.pdf)
  pdf_path <- file.path(output_dir, paste0(filename, ".pdf"))
  cropped_path <- file.path(output_dir, paste0(filename, "_cropped.pdf"))
  if (file.exists(cropped_path)) {
    pdf_path <- cropped_path
  }
  if (!file.exists(pdf_path)) return(invisible(NULL))
  if (knitr::is_latex_output()) {
    cat(sprintf("\n\n\\centering\\includegraphics{%s}\n\n", pdf_path))
  } else {
...
wrap_table <- function(filename, position = "r", width = "0.45\\textwidth") {
  ...
  if (!file.exists(pdf_path)) return(knitr::asis_output(""))
  knitr::asis_output(sprintf("\\begin{wrapfigure}{%s}{%s}
  \\raggedleft
  \\includegraphics{%s}
\\end{wrapfigure}\\noindent", position, width, pdf_path))
}
```

This quickstart guide covers the key features in zzobj2fig v0.2.1. For complete
...
```{r basic-display, echo=FALSE, eval=has_latex, results='asis'}
...
- `basic_table.pdf` - Cropped PDF (primary output for document inclusion)
- `basic_table_full.pdf` - Full PDF (before cropping)


## Interactive Terminal Graphics (zzvim-R)
When working in a kitty-compatible terminal (Kitty, Ghostty, WezTerm, iTerm2)
with zzvim-R, you can preview tables directly in the terminal using
`show_table()`:
...
Tables and plots share a unified history. Use `show_history("table")` to filter
to tables only, or `show_history("plot")` for plots.

Export options:

```{r show-export, eval=FALSE}
...
For reproducible documents, use `o2f()` with `graph_include()`:
```{r graph-include-example, eval=FALSE}
# In an Rmd/qmd code chunk:
o2f(model, theme = "nejm") |> graph_include()
```

The `o2f()` function returns the path to the generated PDF, which
`graph_include()` passes to `knitr::include_graphics()`.

This is the recommended workflow for batch rendering:

1. **Interactive exploration**: Use `show_table()` to preview and compare tables
2. **Final document**: Use `o2f() |> graph_include()` for reproducible embedding
...
```{r themes-display, echo=FALSE, eval=has_latex, results='asis'}
...
```{r stats-display, echo=FALSE, eval=has_latex, results='asis'}
include_table("regression")
```

Supported object types include `lm`, `glm`, `anova`, `aov`, `htest`, `matrix`,
`table`, plus rms objects (`ols`, `lrm`, `cph`, `orm`, `Glm`, `psm`) and
additional types via broom integration (coxph, survreg, nls, polr, prcomp,
kmeans, lmerMod, and more).
...
To wrap paragraph text around a table, use the `wrapfig` LaTeX package. First,
add `\usepackage{wrapfig}` to your YAML header, then use inline R code with
`knitr::asis_output()`:

```{r wrap-helper, eval=FALSE}
wrap_table <- function(filename, position = "r", width = "0.45\\textwidth") {
...
```{r wrap-demo-table, include=FALSE, eval=has_latex}
o2f(mtcars[1:4, 1:3], theme = "nejm",
    filename = "wrap_demo", sub_dir = output_dir)
```

Then embed the table inline using backtick-r syntax. The following paragraph demonstrates text wrapping:
`r if (has_latex) wrap_table("wrap_demo", width = "0.38\\textwidth")`
This text flows around the table on the right. The position parameter controls placement: "r" for right, "l" for left. The width parameter sets how much horizontal space the table occupies. Text continues to wrap naturally until sufficient content fills the space beside the table. This approach works well for small summary tables that complement the narrative without interrupting the reading flow. Notice how the paragraph continues alongside the table, making efficient use of page space. When the text extends beyond the table height, it returns to full width automatically. This technique is particularly useful in research papers where you want to discuss results while displaying the supporting data in close proximity to the discussion. Additional text can continue here to demonstrate that normal formatting resumes after the wrapped section.
...
```{r regression-display, echo=FALSE, eval=has_latex, results='asis'}
...
```{r rms-compare, eval=FALSE}
m1 <- ols(mpg ~ wt, data = mtcars)
m2 <- ols(mpg ~ wt + cyl, data = mtcars)
o2f_rms_compare(Simple = m1, Full = m2, filename = "rms_comparison")
```

RMS methods: `o2f.ols()`, `o2f.lrm()`, `o2f.cph()`, `o2f.orm()`, `o2f.Glm()`,
`o2f.psm()`, and `o2f_rms_compare()`.


## Extensibility: Any Broom Object

For any object supported by the broom package, use `o2f_tidy()`:

```{r o2f-tidy, eval=FALSE}
library(survival)
...
```{r caption-footnote, eval=has_latex}
fn <- o2f_footnote(
...
```{r caption-footnote-display, echo=FALSE, eval=has_latex, results='asis'}
...
Group columns under spanning headers using `o2f_header_above()`:

```{r header-above, eval=has_latex}
...
```{r header-above-display, echo=FALSE, eval=has_latex, results='asis'}
...
```{r scolor-display, echo=FALSE, eval=has_latex, results='asis'}
...
```{r decimal-display, echo=FALSE, eval=has_latex, results='asis'}
...
| `lrm` | `o2f.lrm()` | Logistic regression (OR) |
| `cph` | `o2f.cph()` | Cox proportional hazards (HR) |
| `orm` | `o2f.orm()` | Ordinal regression |
| `Glm` | `o2f.Glm()` | Generalized linear model |
| `psm` | `o2f.psm()` | Parametric survival |

All rms methods support `output = "coef"` (default) or `output = "anova"` for
chunk tests, and `exponentiate = TRUE` for odds/hazard ratios.
...
Use `o2f_tidy()` for any object type supported by broom.

### Theme Presets

| Function | Style |
|:---------|:------|
| `o2f_theme_nejm()` | NEJM (medical journals) |
| `o2f_theme_apa()` | APA (psychology) |
| `o2f_theme_nature()` | Nature (scientific) |
| `o2f_theme_minimal()` | Clean, minimal |
