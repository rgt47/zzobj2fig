...
#'
#' @description Functions for specifying cell-level formatting in o2f tables.
...
#'
#' @return A o2f_format object (list with class "o2f_format").
#'
#' @examples
#' \dontrun{
#' # Bold specific cells
...
#'
#' @return A o2f_format object with the condition and styling specified.
#'
#' @examples
#' \dontrun{
...
#'
#' @export
o2f_highlight <- function(condition,
...
#'
#' @return A o2f_format object with bold=TRUE for specified columns.
#'
#' @examples
#' \dontrun{
...
#'
#' @export
o2f_bold_col <- function(cols) {
  if (is.null(cols) || length(cols) == 0) {
    stop("`cols` must be a non-empty vector.", call. = FALSE)
  }
  o2f_format(cols = cols, bold = TRUE)
}
...
#' @param cols Integer vector or character vector of column indices or names.
#'
#' @return A o2f_format object with italic=TRUE for specified columns.
#'
#' @examples
#' \dontrun{
#' o2f_italic_col("variable")
#' }
#'
#' @export
o2f_italic_col <- function(cols) {
  if (is.null(cols) || length(cols) == 0) {
    stop("`cols` must be a non-empty vector.", call. = FALSE)
  }
  o2f_format(cols = cols, italic = TRUE)
}
...
#'
#' @return A o2f_format object with specified background for rows.
#'
#' @examples
#' \dontrun{
...
#'
#' @export
o2f_color_row <- function(rows, background) {
  ...
  o2f_format(rows = rows, background = background)
}

#' Print method for o2f_format objects
#'
#' @param x A o2f_format object.
#' @param ... Additional arguments (ignored).
#' @return Invisibly returns x.
#' @export
print.o2f_format <- function(x, ...) {
  cat("o2f formatting specification:\n")
...
#'
#' @description Internal function that applies a list of o2f_format objects
...
#' @param formatting A list of o2f_format objects.
#'
...
apply_formatting <- function(kable_obj, df, formatting) {
  ...
  if (inherits(formatting, "o2f_format")) {
    formatting <- list(formatting)
  }

  for (fmt in formatting) {
    if (!inherits(fmt, "o2f_format")) {
      warning("Skipping non-o2f_format object in formatting list.")
      next
    }
